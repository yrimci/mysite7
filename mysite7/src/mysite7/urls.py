...
urls.py: 웹 프로젝트가 실행될 때 웹클라이언트가 요청을 한 인터넷주소에 해당하는 뷰클래스/함수를 호출하기 위한 등록 파일

뷰클래스/함수를 등록할 때는 urlpatterns 변수의 요소로 추가하면 됨
...
#path(시작문자열, include(urls.py가 있는 위치))->시작문자열로 요청하는 모든 인터넷주소는 include안에 있는 urls.py가 처리할 수 있도록 등록
#ex) path('test/', include('vote.urls'))->127.0.0.1:8000/test/로 시작하는 모든 요청들을 vote폴더에 urls.py에서 처리할 수 있도록 등록


#등록할 뷰함수를 import
#vote/views.py에 존재하는 test함수 import
...
urlpatterns = [
    ...
    #127.0.0.1:8000/숫자/로 요청하는 처리는 test_input함수를 호출!
    #호출할 때, 숫자값을 number변수에 인자값으로 사용
    path('<int:number>/', test_input),
    #path함수에 name매개변수: 등록된 뷰의 별칭을 등록하는 매개변수
    #템플릿: {% url 별칭의이름(문자열) %}
    ...
    #127.0.0.1:8000/auth/로 시작하는 요청들을 social_django 어플리케이션의 urls.py에서 처리
    #include함수의 namespace 매개변수: 해당 urls.py에 지정된 app_name 값을 사용하지 않고 새로운 그룹명을 지정
